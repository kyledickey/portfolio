...
type Painter = (t: number, tone: Float32Array) => void;
...
/** Lets the time-of-day preview under the print pick an hour. */
...
function previewHour(hour: number | null) {
    hourOverride = hour;
}

const PATTERNS = [peaks, marble, moire, tiles, life];
const STORAGE_KEY = "print-pattern";

/**
 * A rectangle of moving dithered pattern, printed straight onto the page,
 * with my name cut out of it.
 */
export function CobaltPrint() {
    ...
    // Remember the viewer's pick, if their browser lets us.
    useEffect(() => {
        try {
